...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
        <div class="mb-8 flex justify-between items-center">
            <div>
                 <h2 class="text-2xl font-bold text-gray-900 font-['Outfit']">Edit Room</h2>
                <p class="mt-2 text-gray-500">Update details for room ID #{{ $room->id }}</p>
            </div>
             <form action="{{ route('rooms.delete', $room->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this room?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="text-red-600 hover:text-red-900 font-medium text-sm border border-red-200 bg-red-50 hover:bg-red-100 px-4 py-2 rounded-lg transition">
                    Delete Room
                </button>
            </form>
        </div>

        <form method="POST" action="{{ route('rooms.update', $room->id) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                 <div>
                    ...
                    <input type="number" name="beds" id="beds" min="1" value="{{ old('beds', $room->beds) }}" required
                           class="block w-full px-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-[#9A616D] focus:border-[#9A616D] transition sm:text-sm">
                </div>
                 <div>
                     <label for="washrooms" class="block text-sm font-medium text-gray-700 mb-1">Washrooms</label>
                    <input type="number" name="washrooms" id="washrooms" min="1" value="{{ old('washrooms', $room->washrooms) }}" required
                           class="block w-full px-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-[#9A616D] focus:border-[#9A616D] transition sm:text-sm">
                </div>
                 <div>
                     <label for="guests" class="block text-sm font-medium text-gray-700 mb-1">Max Guests</label>
                    <input type="number" name="guests" id="guests" min="1" value="{{ old('guests', $room->guests) }}" required
                           class="block w-full px-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-[#9A616D] focus:border-[#9A616D] transition sm:text-sm">
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative rounded-xl shadow-sm">
                    ...
                    <input type="number" name="price" id="price" min="0" value="{{ old('price', $room->price) }}" required
                           class="block w-full pl-14 pr-12 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-[#9A616D] focus:border-[#9A616D] transition sm:text-sm">
                </div>
            </div>
            ...
            <div>
                 ...
                 @if($room->image)
                    <div class="mb-4">
                        <img src="{{ asset('storage/' . $room->image) }}" alt="Current Image" class="h-32 w-auto rounded-lg object-cover">
                    </div>
                 @endif
            ...
            <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                     @php
                        $currentAmenities = is_string($room->amenities) ? json_decode($room->amenities, true) : ($room->amenities ?? []);
                        ...
                        <label class="inline-flex items-center space-x-2 cursor-pointer">
                            <input type="checkbox" name="amenities[]" value="{{ $amenity }}" 
                                  {{ in_array($amenity, $currentAmenities) ? 'checked' : '' }}
                                   class="rounded text-[#9A616D] focus:ring-[#9A616D] border-gray-300">
            ...
            <div class="pt-6 flex gap-4">
                 ...
                 <a href="{{ route('rooms.index') }}" class="flex-none justify-center py-3 px-6 border border-gray-300 rounded-xl shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#9A616D] transition">
                    Cancel
                </a>
            </div>
        </form>
